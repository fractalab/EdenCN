...
namespace eden
{
   ...
   void election_seeder::update(eosio::input_stream& bytes)
   {
      ...
      bytes.skip(4 + 4);
      //Seed block is too early
      //{ "start_time": "2022-05-26T11:00:01.000", "election_threshold": 10001, "election_schedule_version": 2 }
   ...
   void elections::set_default_election(eosio::time_point_sec origin_time)
   {
      ...
      set_state_sing(current_election_state_registration_v1{
          get_election_time(state.election_start_time, origin_time + eosio::days(3)),
          new_threshold});
   }
