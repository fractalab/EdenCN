...
export const getTemplates = async (
    ...
    //chenke limit 改为固定值20
    let url = `${atomicAssets.apiBaseUrl}/templates?collection_name=${atomicAssets.collection}&schema_name=${atomicAssets.schema}&page=${page}&limit=20&order=${order}&sort=${sortField}${FETCH_AFTER_TIMESTAMP}`;
    //alert(url)
    if (ids.length) {
        url += `&ids=${ids.join(",")}`;
    }
